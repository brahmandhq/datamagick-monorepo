...
export default function TeamConnectionModal(props) {
    ...
    const storeDbInfo = async (name: string, db: string, dbData: string[], teamId: string, email: string) => {
    ...
    const onSubmit = async (values: z.infer<typeof connectionSchema>) => {
        ...
        try {
            ...
            await storeDbInfo(values.name, values.db, dbdata, values.teamId, data?.user?.email);
