...
export default function CreateConnectionModal(props) {
  ...
  const onSubmit = async (values: z.infer<typeof connectionSchema>) => {
    ...
    try {
      ...
      await storeDbInfo(values.name, values.db, dbdata, data?.user?.email);
      ...
      toast.error("Error occurred while creating Connection", {
        position: "top-right",
      });
    } finally {
      setStatus('');
